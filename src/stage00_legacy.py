"""Original Stage 00 data, configuration and diagnostic entry points.

Kept separately to reproduce the historical (not leakage-free) smoke protocol.
"""
...
def _load_forecast_loaders(data_path, splits=("train", "test")):
    ...
    return tuple(get_evaluation_loaders(split=split, **loader_kwargs) for split in splits)
...
def _run_command(cmd, cwd, timeout=120, log_path=None):
    ...
    if log_path is not None:
        ...
        Path(log_path).with_suffix(".json").write_text(json.dumps({
            "argv": list(map(str, cmd)), "cwd": str(cwd),
...
def _pretrain_smoke_case(workdir, data_name, rows, python_executable=None, extra_args=(), log_path=None):
...
def _pretrain_mnist_row_case(workdir, mnist_root, python_executable=None, extra_args=(), log_path=None):
    python_executable = python_executable or sys.executable
    output = _run_command(
        [
            python_executable,
